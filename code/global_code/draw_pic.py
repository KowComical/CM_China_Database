...
for category_name in sector_list:
    ...
    for co, i in zip(pro_list, num):
        ...
        # 列转行
        min_max = pd.pivot_table(df_rest, index='year', values=category_name, columns='month_date').reset_index().drop(
            columns=['year'])
        min_list = []
        max_list = []
        date_list = []
        ...
        ax = df_max.set_index('month_date')['max'].plot(color='tab:brown', linewidth=10, alpha=0, label='_nolegend_')
        df_min.set_index('month_date')['min'].plot(ax=ax, color='tab:grey', linewidth=10, alpha=0, label='_nolegend_')
