...
def main():
    ...
    # 两部分信息
    first_part = re.compile(r'<div class="r col-sm-5 col-md-6 col-xs-12">.*?<ol class="num"><li>(?P<first>.*?)</li>',
                            re.S)
    second_part = re.compile(r'<li class="col-xs-12 col-md-6">.*?<a href="/upc/(?P<num>.*?)".*?<p>(?P<name>.*?)</p>',
                             re.S)
